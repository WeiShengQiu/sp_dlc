...
function UpdateUnitPortrait( pUnit )
	...
	and GameInfo.Unit_FreePromotions{ UnitType = GameInfo.Units[pUnit:GetUnitType()].Type, PromotionType = "PROMOTION_CORPS_2" }() == nil
	then
	    ...
	    elseif pUnit:GetDomainType() == DomainTypes.DOMAIN_SEA then
	      if GameInfo.Leader_Traits{ LeaderType = GameInfo.Leaders[Players[pUnit:GetOwner()]:GetLeaderType()].Type, TraitType = "TRAIT_FIGHT_WELL_DAMAGED" }()
	...
	and GameInfo.Unit_FreePromotions{ UnitType = GameInfo.Units[pUnit:GetUnitType()].Type, PromotionType = "PROMOTION_CORPS_1" }() == nil
	then
	    if     pUnit:GetDomainType() == DomainTypes.DOMAIN_LAND then
								...
								if (pUnit ~= nil and not pUnit:IsInvisible(iTeam, false)) then
									...
									--if (pUnit:GetDomainType() ~= DomainTypes.DOMAIN_AIR) then
										...
										if (pHeadUnit:GetDomainType() == pUnit:GetDomainType() or pHeadUnit:IsRanged() or (pHeadUnit:GetDomainType() == DomainTypes.DOMAIN_SEA and pUnit:IsEmbarked())) then
											...
											if (pUnit:GetBaseCombatStrength() > 0 or pHeadUnit:IsRanged()) and pUnit:GetMaxDefenseStrength(pPlot,pHeadUnit) > iMaxDefenseStrength then
