...
function NewAttackEffect()
		...
		if IsMoveToAdjOnly then
			...
			if attUnit:GetMoves() >= GameDefines["MOVE_DENOMINATOR"] then
				if iExtraAttacks > 1 then
				    ...
				    local iMarkNum =math.min(math.ceil(coeff*attUnit:GetMoves()/GameDefines["MOVE_DENOMINATOR"]),iExtraAttacks)-1;
				    if     iMarkNum == 10 then
	...
	then
		...
		if attPlayer:IsHuman() then
			Events.GameplayAlertMessage( Locale.ConvertTextKey( "TXT_KEY_SP_NOTIFICATION_ATTACKING_CITY_LOST_MOVEMENT", attUnit:GetName()) )
		end
	end
	...
	-- Special Forces sabotage city 
		...
		if defCity:GetDamage() < defCity:GetMaxHitPoints() and unitCount > 0 then
			...
			for i = 0, unitCount-1, 1 do
				...
				if pFoundUnit and not pFoundUnit:IsHasPromotion(SpeComID) then
					...
					if attPlayer:IsHuman() then
						Events.GameplayAlertMessage( Locale.ConvertTextKey( "TXT_KEY_SP_NOTIFICATION_SPFORCE_CITY_SABOTAGE", pFoundUnit:GetName()) )
					end
				end
			end
		end
		...
		-------------------By 7----------------------
		--[[if attUnit:IsDead() and attUnit:IsHasPromotion(GameInfo.UnitPromotions["PROMOTION_SP_FORCE_2"].ID)
		and attUnit:IsHasPromotion(GameInfo.UnitPromotions["PROMOTION_SPFORCE_2"].ID) then
			local unitType = attUnit:GetUnitType()
			local unitEXP = attUnit:GetExperience()
			local unitAIType = attUnit:GetUnitAIType()
			local unitX = attUnit:GetX()
			local unitY = attUnit:GetY()
			local NewUnit = attPlayer:InitUnit(unitType, unitX, unitY, unitAIType)
...
--		defCity:ChangeResistanceTurns(1)
--		
--		if defCity:GetGarrisonedUnit() == nil  then
--			local CityMaxHP = defCity:GetMaxHitPoints()
--			if CityMaxHP >= 250 then
--				defCity:SetDamage (250)
--			else
--				defCity:SetDamage (CityMaxHP)
--			end
--		end
	...
	------- Fix Strong Unit cannot capture city Bug(damage overflow)
	    if not attUnit:IsDead() and (attUnit:GetUnitCombatType()== GameInfoTypes.UNITCOMBAT_NAVALMELEE or attUnit:GetUnitCombatType()== GameInfoTypes.UNITCOMBAT_ARMOR or attUnit:IsHasPromotion(GunpowderInfantryUnitID) 
		...
		if attUnit:GetBaseCombatStrength()/(defCity:GetStrengthValue() / 100)>=11 and defCity:GetPopulation()<15 and IsAdjacentToDefendingCity then
...
--		attPlayer:AcquireCity(defCity)
	    ...
	    ----------------Militia can capture the resisting city in one hit
--	    local resTime = defCity:GetResistanceTurns()
--	    if attUnit:GetUnitCombatType()== GameInfoTypes.UNITCOMBAT_RECON and resTime >= 1 then
--		
--		local TempUnit = attPlayer:InitUnit(GameInfoTypes.UNIT_ROMAN_LEGION, plotX, plotY, UNITAI_ATTACK)
--		TempUnit:Kill()
--		attUnit:SetXY(plotX, plotY)
		...
		if  ( defCity:GetDamage() >= defCity:GetMaxHitPoints() - 1 ) then
			...
			if ( cityPop > 1 ) then
				...
				if Players[CityOwner]:IsHuman() then
					...
					local text = Locale.ConvertTextKey("TXT_KEY_SP_NOTIFICATION_CITY_POPULATION_LOST_BY_RANGEDFIRE", attUnit:GetName(), defCity:GetName())
...
--			else
----				if defCity:IsHasBuilding(GameInfo.Buildings["BUILDING_WALLS"].ID) then----------Destroy a city with only 1 pop and without wall (heavily damaged)
----					return
----				elseif defCity:IsOriginalCapital()then
----					return
----				else
----					defCity:Kill()
...
--				end
		...
		-- Notification
		if attPlayer:IsHuman() then
		 	local text = Locale.ConvertTextKey( "TXT_KEY_SP_NOTIFICATION_BOMBER_CITY_KILL_POPULATION_ATTACKING", tostring(CityPopLoss))
			Events.GameplayAlertMessage( text )
		end
		if defPlayer:IsHuman() then
			local text = Locale.ConvertTextKey("TXT_KEY_SP_NOTIFICATION_BOMBER_CITY_KILL_POPULATION_ATTACKED", tostring(CityPopLoss), defCity:GetName())
		...
		if batPlot and defCity then 
			...
			---------------------Notification
			if attPlayer:IsHuman() then
				local text = Locale.ConvertTextKey( "TXT_KEY_SP_NOTIFICATION_BOMBER_CITY_BUILDING_DESTROYED_ATTACKING", tostring(BuildingLoss))
				Events.GameplayAlertMessage( text )
			end
			
			if defPlayer:IsHuman() then
				local text = Locale.ConvertTextKey("TXT_KEY_SP_NOTIFICATION_BOMBER_CITY_BUILDING_DESTROYED_ATTACKED", tostring(CityPopLoss), defCity:GetName())
				local heading = Locale.ConvertTextKey("TXT_KEY_SP_NOTIFICATION_BOMBER_CITY_ATTACKED_SHORT", defCity:GetName())
				defPlayer:AddNotification(NotificationTypes.NOTIFICATION_STARVING, text, heading, plotX, plotY)
			end
		end
	...
	-- Attacking a Unit!
	elseif defUnit then
	    -- fix "defUnitDamage" for 100+ HP
	    if attUnit and defUnit and not defUnit:IsDead() and not defUnit:IsDelayedDeath() and (defFinalUnitDamage >= 100 or defUnitDamage <= 0)
	    and GameInfoTypes["PROMOTION_MAXHP_REDUCE_1"] and GameInfoTypes["PROMOTION_MAXHP_REDUCE_2"]
	    and GameInfoTypes["PROMOTION_MAXHP_REDUCE_3"] and GameInfoTypes["PROMOTION_MAXHP_REDUCE_4"]
	    then
		local fixedDefUnitDamage = 1;
		if batType == GameInfoTypes["BATTLETYPE_MELEE"] then
			fixedDefUnitDamage = math.max(1, defUnit:GetCombatDamage(defUnit:GetMaxDefenseStrength(batPlot, attUnit), attUnit:GetMaxAttackStrength(attPlot, batPlot, defUnit), defUnit:GetDamage(), false, false, false) - defUnitDamage);
		else
			fixedDefUnitDamage = math.max(1, attUnit:GetRangeCombatDamage(defUnit, nil, false) - defUnitDamage);
		end
		if defUnitDamage ~= fixedDefUnitDamage then
			defFinalUnitDamage = defFinalUnitDamage - defUnitDamage + fixedDefUnitDamage;
			defUnitDamage = fixedDefUnitDamage;
		end
	    end
	    
	    ------ Collateral damage (both melee and ranged)!
	    if (attUnit:IsHasPromotion(NavalRangedShipUnitID) or attUnit:IsHasPromotion(NavalRangedCruiserUnitID) or attUnit:IsHasPromotion(CitySiegeUnitID)) and batPlot:GetNumUnits() > 1 then
...
-----------Attacking with debuffs
	    if(attUnit:IsHasPromotion(Sunder1ID) or attUnit:IsHasPromotion(Barrage1ID) or attUnit:IsHasPromotion(CollDamageLV1ID) 
	    or attUnit:IsHasPromotion(DestroySupply_CarrierID) or attUnit:IsHasPromotion(DestroySupply1ID) or attUnit:IsHasPromotion(SPForce1ID)
...
--		print("Moves Left:"..MovesLeft);
		...
		local tdebuff = nil;
		local tlostHP = nil;
		if   ( attUnit:IsHasPromotion(DestroySupply1ID) or attUnit:IsHasPromotion(SPForce1ID) or attUnit:IsHasPromotion(DestroySupply_CarrierID) )
		...
		if (defFinalUnitDamage/defUnit:GetMaxHitPoints() > 0.4 ) then
    		...
    		elseif (defFinalUnitDamage/defUnit:GetMaxHitPoints() > 0.25 and defFinalUnitDamage/defUnit:GetMaxHitPoints() <= 0.4 ) then 
    		...
    		elseif (defFinalUnitDamage/defUnit:GetMaxHitPoints() > 0.1 and defFinalUnitDamage/defUnit:GetMaxHitPoints() <= 0.25 ) then 
			...
			or attUnit:IsHasPromotion(SubmarineUnitID)) and defUnit:IsHasPromotion(GameInfo.UnitPromotions["PROMOTION_MOVEMENT_LOST_1"].ID, true) then
    			if defUnit:CanMove() then
				IsNotification = true;
				defUnit:SetMoves(0)
			end
...
function SetRealFinalDamageAndDamageLevel(pUnit, iDamage)
	...
	--	end
		...
		if pUnit:IsHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_1"]) then
			iDamageLevel = iDamageLevel + 1;
		end
		if pUnit:IsHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_2"]) then
			iDamageLevel = iDamageLevel + 2;
		end
		if pUnit:IsHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_3"]) then
			iDamageLevel = iDamageLevel + 3;
		end
		if pUnit:IsHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_4"]) then
			iDamageLevel = iDamageLevel + 4;
		end
		...
		elseif iDamageLevel == 4 then
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_4"], true);
		elseif iDamageLevel == 5 then
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_1"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_4"], true);
		elseif iDamageLevel == 6 then
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_2"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_4"], true);
		elseif iDamageLevel == 7 then
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_3"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_4"], true);
		elseif iDamageLevel == 8 then
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_1"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_3"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_4"], true);
		elseif iDamageLevel == 9 then
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_2"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_3"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_4"], true);
		else	-- if iDamageLevel >  9 then
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_1"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_2"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_3"], true);
			pUnit:SetHasPromotion(GameInfoTypes["PROMOTION_MAXHP_REDUCE_4"], true);
...
--	   		pFoundUnit:SetMoves(GameDefines["MOVE_DENOMINATOR"])	
